...
Given widespread availability of tools and comprehensive guides already published on the subject, it's clear to me ***the biggest obstacle to writing one's first R package is its*** **perceived difficulty**. And since every new blog post reshines the light on the amazing resources that already exist, I've written yet another blog post about *how to write an R package.*

## Useful Tools and Guides for Creating R Packages

On the topic of *how to write an R package*, there are already several well-established tools (e.g., the [{devtools}](https://devtools.r-lib.org/) and [{usethis}](https://usethis.r-lib.org/) packages) and comprehensive guides (e.g., [Writing R Extensions by R Core Team](https://cran.r-project.org/doc/manuals/r-release/R-exts.html) and [R packages by Hadley Wickham](http://r-pkgs.had.co.nz/)). There are even several demystifying blog posts on the subject!

One of the first (and probably most famous) blog posts is by [Hilary Parker](https://twitter.com/hspter), who was nice enough to not only provide a timeless guide on writing an R package but also identify, number, and title the exact steps in the process:
...
The steps outlined above by Hilary Parker still essentially hold true today, but like most things related to software development, they don't incorporate the newest (and in this case I think best) tools. For that, you can turn the post written by [Tomas Westlake](https://twitter.com/tomwestlake121), which in addition to adding a few more steps (up to 10 steps instead of 6) also provides more timely references to useful developer tools:
...
In addition to these *step-by-step*/*how-to* posts on how to write an R package, there is also a fantastic blog post written by [Emil Hvitfedt](https://twitter.com/hvitfeldt) on some additional tasks associated with testing, documenting, and maintaining R packages:
...
as well as an excellent evaluative blog post written by [Maëlle Salmon](https://twitter.com/ma_salmon) about what goes into *good* **open-science** R packages:
...
+ What will your package be called? `available::available()`
+ How to build an R package? [rOpenSci Developer Guide](https://ropensci.github.io/dev_guide)
+ Where should your package live?
+ How to check your package automatically `devtools::check()` `styler::style()`
+ Bring the package to its users and get feedback `citation()` [Journal of Open Source Software](http://joss.theoj.org/)

If the goal is to provide guidance on how to go from zero to sixty on writing an R package, it's really tough to improve on these posts.
